...
//   const [isLoading, setIsLoading] = useState(false);
//   const [email, setEmail] = useState<string | null>(null); // 初期値を null にする
//   const router = useRouter();
...
//   useEffect(() => {
//     if (typeof window !== 'undefined') {
//       // sessionStorage の使用を無効化
//       // const storedEmail = sessionStorage.getItem('email');
//       // setEmail(storedEmail || ''); // null の場合は空文字をセット
//     }
//   }, []);
...
//                     placeholder="name@example.com"
//                     disabled={isLoading}
//                     defaultValue={email || ''} // email の値が null の場合は空文字
//                     className="backdrop-blur-sm bg-white/50 border-black/50"
//                     required
//                   />
//                 </div>
...
function SignInContent() {
  const [isLoading, setIsLoading] = useState(false);
  const [email, setEmail] = useState(''); // sessionStorageを使わずにuseStateで管理
  const router = useRouter();
  const searchParams = useSearchParams();
  const callbackUrl = searchParams.get('callbackUrl') || '/profile';
